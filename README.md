...
| **한계** | 자동화 테스트 없음, 녹음 URL 24시간 만료, 한 요청에 여러 외부 호출을 순서대로 처리 ([한계와 개선 과제](#-한계와-개선-과제)) |

![대시보드](docs/images/dashboard.png)

> 대시보드 화면(로컬 실행, 2026-09-19). 공고 분석 · 자소서 피드백 · 음성 면접 화면은 OpenAI · S3 키가 필요해 이번에 싣지 못했습니다.
...
| 5. 포트폴리오 정리 (사후) | 개발 기간 이후 작업. 보안 점검(하드코딩 JWT 시크릿 · 민감 정보 로그 · 무인증 관리자 API 제거, `/api/interview/start`의 와일드카드 CORS와 응답 속 stack · 요청 본문 노출 제거), 문서 재분류와 키 · 버킷 · 도메인 마스킹, 일회성 스크립트 삭제, `.env.example` 추가 |
...
- **Presigned URL 만료**: DB에 24시간짜리 Presigned URL을 그대로 저장해, 하루가 지나면 공고 원본 PDF 링크와 답변 녹음 재생이 동작하지 않습니다. S3 key를 저장하고 조회 시점에 URL을 발급하도록 바꿔야 합니다.
- **토큰 저장 위치**: JWT를 `localStorage`에 저장해 XSS에 취약합니다. httpOnly 쿠키가 더 안전합니다.
- **긴 동기 요청**: 답변 1건마다 S3 업로드 → Whisper → GPT → TTS → S3 업로드를 한 요청에서 순서대로 처리하고, 5번째 답변은 전체 피드백 생성까지 60초 안에 끝내야 합니다. 비동기 작업 큐나 스트리밍이 필요합니다.
- **브라우저 호환성**: 녹음 형식을 `audio/webm`으로 고정해 Safari 지원이 불확실합니다.
- **자동화 테스트 부재**: API·프롬프트 출력 형식에 대한 테스트가 없습니다.
- **스키마 이중 관리**: Prisma 스키마가 `schema.sql`과 일부 다릅니다(예: `interview_turns.feedback_text` 타입). 쓰지 않는 코드(`ProfileForm`, `zod`, 업로드용 Presigned URL 함수)도 남아 있습니다.
