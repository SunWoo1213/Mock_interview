...
- **Presigned URL 만료 → 해결(사후)**: 업로드 시 발급한 24시간짜리 Presigned URL을 DB에 저장해 하루가 지나면 공고 PDF와 답변 녹음이 열리지 않았습니다. 조회 API(면접 결과 · 공고 상세)가 저장된 URL에서 키를 꺼내 매번 다시 서명하도록 바꿨고, 만료 서명 URL은 403 · 재서명 URL은 200인 것을 실제 S3로 확인했습니다. 저장 형식을 S3 키로 바꾸는 것은 남은 과제입니다.
...
- **스키마 이중 관리**: Prisma 스키마는 런타임에서 쓰지 않는 문서용이라 `schema.sql`과 따로 관리됩니다. 어긋나 있던 `interview_turns.feedback_text` 타입(JSONB)은 맞췄고, 쓰지 않던 `ProfileForm` · `zod` · 업로드용 Presigned URL 함수는 삭제했습니다.
