...
### 2. 채용 공고 분석
- PDF 업로드 또는 텍스트 직접 입력
- 자동 텍스트 추출 (pdf-parse)
- GPT-4o로 핵심 키워드, 필수/우대 요건 자동 분석
- S3에 원본 PDF 저장
- **공고 히스토리 관리**: 저장된 공고 조회, 선택, 삭제 기능

### 3. 자기소개서 피드백
- **공고 선택 페이지**: 분석된 공고 중 선택하여 작성 시작
- **Split View UI**: 좌측에 공고 분석 결과, 우측에 작성 영역
  - 필수 요건, 핵심 키워드를 참고하며 실시간 작성
  - 작성 팁 및 가이드 제공
- 사용자 스펙 + 채용 공고 + 자소서 통합 분석
- AI가 강점, 약점, 상세 분석, 즉시 적용 가능한 수정 예시 생성
- 구조화된 JSON 피드백 제공
- 예상 면접 질문 생성
