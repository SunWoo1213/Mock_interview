...
### 방법 1: API 엔드포인트를 통한 마이그레이션 (삭제됨)

> ⚠️ 이 엔드포인트는 인증 없이 DDL을 실행할 수 있어 보안상 위험하므로 마이그레이션 완료 후 삭제되었습니다.
> 현재는 `node scripts/run-add-voice-column.js` 또는 방법 3(SQL 직접 실행)을 사용하세요. 아래는 당시 작업 기록입니다.
...
- `database/migrations/add-voice-column.sql` - 마이그레이션 SQL
