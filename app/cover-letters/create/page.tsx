    ...
    return (
      <div className="min-h-screen bg-zinc-50">
        <div className="max-w-4xl mx-auto px-8 py-16 animate-fade-in">
          <button
            onClick={() => router.push('/')}
            className="mb-8 text-zinc-600 hover:text-zinc-900 transition-colors flex items-center gap-2"
          >
            <span>←</span>
            <span>대시보드로 돌아가기</span>
          </button>

          <div className="text-center py-16 bg-white rounded-2xl shadow-sm border border-zinc-200 px-8">
            <div className="mb-8">
              ...
              <h1 className="text-4xl font-black mb-4 text-zinc-900">자기소개서 작성</h1>
            ...
            <div className="max-w-2xl mx-auto space-y-4">
              <button
                onClick={() => router.push('/')}
                className="w-full px-8 py-5 bg-zinc-900 hover:bg-zinc-800 text-white font-bold text-lg rounded-xl transition-all shadow-sm active:scale-95"
              >
              ...
              <button
                ...
                className="w-full px-8 py-5 bg-white hover:bg-zinc-50 text-zinc-900 font-bold text-lg rounded-xl transition-all border border-zinc-200 hover:border-zinc-300 shadow-sm"
              >
                <div className="flex items-center justify-center gap-3">
                  <span className="text-2xl">➕</span>
    ...
    <div className="max-w-[1800px] mx-auto px-6 py-8">
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                {jobPosting?.companyName && jobPosting?.title && (
                  <p className="text-sm text-zinc-600">{jobPosting.companyName}</p>
                )}
              </div>
            </div>
            {!feedback && (
              <button
                onClick={() => router.push('/cover-letters/select-posting')}
                className="px-4 py-2 bg-white hover:bg-zinc-50 text-zinc-900 text-sm font-medium rounded border border-zinc-200 transition-all"
              >
                공고 변경
              </button>
            )}
      ...
      {!feedback ? (
        ...
        <div className="flex h-[calc(100vh-120px)]">
          {/* 왼쪽: 공고 분석 결과 (참고용) */}
          <div className="w-2/5 border-r border-zinc-200 overflow-y-auto bg-zinc-50">
            <div className="p-6">
              <div className="mb-6">
                <h2 className="text-2xl font-bold mb-2 flex items-center gap-2 text-zinc-900">
                  <span>📋</span> 공고 분석 결과
                </h2>
          ...
          <div className="flex-1 overflow-y-auto bg-white">
            <form onSubmit={handleSubmit} className="h-full flex flex-col">
              <div className="flex-1 p-6">
                <div className="mb-4">
                  <label className="block text-lg font-bold mb-2 text-zinc-900">
                    ✍️ 자기소개서 작성
                  </label>
                  <p className="text-sm text-zinc-600">
                ...
                <textarea
                  ...
                  className="w-full h-[calc(100vh-280px)] px-4 py-3 bg-white border border-zinc-200 rounded-lg focus:outline-none focus:border-zinc-900 focus:ring-2 focus:ring-zinc-900 transition-all resize-none text-zinc-900 leading-relaxed placeholder:text-zinc-400"
...
- STAR 기법을 활용하세요"
              ...
              <div className="border-t border-zinc-200 p-6 bg-zinc-50">
                <button
                  ...
                  className="w-full px-6 py-4 bg-zinc-900 hover:bg-zinc-800 disabled:bg-zinc-400 disabled:cursor-not-allowed text-white rounded-lg transition-all font-bold text-lg shadow-sm active:scale-95"
                >
        ...
        <div className="max-w-5xl mx-auto px-8 py-12 space-y-8">
                    ...
                    {feedback.strengths.map((strength: any, idx: number) => {
                      ...
                      return (
                        <li key={idx} className="flex items-start gap-3 p-3 bg-green-50 rounded-lg">
                          ...
                          <span className="text-zinc-700 leading-relaxed">{strengthText}</span>
                        </li>
                      );
                    })}
                  </ul>
                    ...
                    {feedback.weaknesses.map((weakness: any, idx: number) => {
                      ...
                      return (
                        <li key={idx} className="flex items-start gap-3 p-3 bg-orange-50 rounded-lg">
                          <span className="text-orange-600 mt-1 font-bold">{idx + 1}.</span>
                          <span className="text-zinc-700 leading-relaxed">{weaknessText}</span>
                        </li>
                      );
                    })}
                  </ul>
              ...
              <div className="p-6 bg-white rounded-2xl border border-zinc-200 shadow-sm">
                ...
                <div className="space-y-4">
                  {feedback.detailedAnalysis.map((analysis: any, idx: number) => (
                    <div key={idx} className="p-4 bg-zinc-50 rounded-lg border-l-4 border-blue-500">
                      <h4 className="font-bold text-blue-600 mb-2">{analysis.section}</h4>
                      <p className="text-zinc-700 leading-relaxed">{analysis.feedback}</p>
                    </div>
                  ))}
                </div>
              </div>
                    ...
                    <div key={idx} className="p-5 bg-zinc-50 rounded-lg border border-zinc-200">
                      <div className="mb-4">
                        ...
                        <div className="p-3 bg-red-50 border-l-4 border-red-400 rounded">
                          <p className="text-zinc-700 italic">&ldquo;{fix.original}&rdquo;</p>
                        </div>
                      </div>
                      <div className="mb-4">
                        <div className="text-sm text-green-700 font-semibold mb-2">✅ 수정 후</div>
                        <div className="p-3 bg-green-50 border-l-4 border-green-400 rounded">
                          <p className="text-zinc-700 font-medium">&ldquo;{fix.improved}&rdquo;</p>
                        </div>
                      </div>
                      <div className="p-3 bg-blue-50 rounded border border-blue-200">
                        <div className="text-sm text-blue-700 font-semibold mb-2">💡 개선 이유</div>
                        <p className="text-zinc-700 text-sm">{fix.reason}</p>
                      </div>
                    </div>
                  ))}
                </div>
                  ...
                  {feedback.interview_questions.map((question: any, idx: number) => {
                    ...
                    return (
                      <li key={idx} className="flex items-start gap-3 p-4 bg-purple-50 rounded-lg border border-purple-200">
                        <span className="text-purple-600 font-bold mt-1">Q{idx + 1}.</span>
                        <span className="text-zinc-700 leading-relaxed">{questionText}</span>
                      </li>
                    );
                  })}
                </ul>
            ...
            <div className="flex gap-4">
              <button
                ...
                className="flex-1 px-6 py-4 bg-zinc-900 hover:bg-zinc-800 rounded-lg transition-all font-semibold text-lg shadow-sm active:scale-95 text-white"
              >
                🎤 모의 면접 시작하기
              </button>
              <button
                onClick={() => router.push('/job-postings/upload')}
                className="px-6 py-4 bg-white hover:bg-zinc-50 rounded-lg transition-all font-semibold border border-zinc-200 shadow-sm"
              >
                다른 공고 작성하기
              </button>
            </div>
